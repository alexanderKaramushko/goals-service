...
`GET /api/v1/users/get-all` возвращает массив локально зарегистрированных
пользователей с полями `id`, `fullName` и `createdAt`. Отдельного endpoint для
получения пользователя по идентификатору нет. Порядок списка не задан.
